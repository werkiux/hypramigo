...
if ! lspci -k | grep -A 2 -E "(VGA|3D)" | grep -iq nvidia; then
    ...
    for ndkms in nvidia-dkms nvidia-utils nvidia-settings libva; do
    ...
    sudo mkinitcpio -p linux 2>&1 | tee -a $LOG
    printf "\n" 
	# Lista negra nouveau.
	...
	sleep 2
	read -n1 -rp "$OPC - GPU NVIDIA detectada. ¿Te gustaría instalar hyprland-nvidia? (s,n) " NVIDIA2
	if [[ $NVIDIA2 =~ ^[Ss]$ ]]; then
    	# Instalar Nvidia Hyprland
    	printf "$NOTA - Instalando hyprland-nvidia..."
    	if pacman -Qs hyprland > /dev/null; then
        	read -n1 -rp "$OPC - Hyprland detectado. ¿Le gustaría eliminarlo e instalar hyprland-nvidia en su lugar? (s,n) " hyprnvidia
        	if [[ $hyprnvidia =~ ^[Ss]$ ]]; then
    		    for hyprnvi in hyprland hyprland-git hyprland-nvidia-hidpi-git; do
        	        sudo pacman -R --noconfirm "$hyprnvi" 2>/dev/null | tee -a $LOG || true
    		    done
    	        yay -S hyprland-nvidia 2>&1 | tee -a $LOG
            fi
        fi
    fi
fi
...
if pacman -Qs sddm > /dev/null; then
    echo
    read -n1 -rep "$OPC - SDDM ya está instalado. ¿Le gustaría desinstalar sddm e instalar sddm-git? (s,n) " sddm_git
    if [[ $sddm_git =~ ^[Ss]$ ]]; then
    ...
    read -n1 -rep "$OPC - ¿Le gustaría instalar el tema sddm catppuccin? (s,n)" temacapu
    if [[ $instal_sddm_catppu =~ ^[Ss]$ ]]; then
        for temacapu in sddm-catppuccin-git; do
            yay -S temacapu 2>&1 | tee -a "$LOG"
            if [ $? -ne 0 ]; then
                echo -e "$ERROR - $temacapu la instalación ha fallado, verifique install.log"
                exit 1
    	    fi
        done
    fi	
...
if pacman -Qs waybar > /dev/null; then
    read -n1 -rep "$OPC - Waybar detectada. ¿Le gustaría desinstalar waybar e instalar waybar-hyprland-git? (s,n)" wbgit
    if [[ $wbgit =~ ^[Ss]$ ]]; then
      sudo pacman -R --noconfirm waybar 2>> "$LOG" > /dev/null || true
    fi
    printf "$NOTA - Instalando waybar-hyprland-git...\n"
    for wayb in waybar-hyprland-git; do
        yay -S $wayb 2>&1 | tee -a "$LOG"
        if [ $? -ne 0 ]; then
            echo -e "$ERROR - $wayb la instalación ha fallado, verifique install.log"
            exit 1
        fi
    done
fi
...
read -n1 -rep "$OPC ¿Le gustaría copiar archivos de configuración y fondos de pantalla? (s,n)" CFG
if [[ $CFG =~ ^[Ss]$ ]]; then
    ...
    cp -r ~/hypramigo/configs/btop ~/.config/ || { echo "Error: no se pudieron copiar los archivos de configuración de btop."; exit 1; } 2>&1 | tee -a "$LOG"
    cp -r ~/hypramigo/configs/cava ~/.config/ || { echo "Error: no se pudieron copiar los archivos de configuración de cava."; exit 1; } 2>&1 | tee -a "$LOG"
    cp -r ~/hypramigo/configs/nwg-look ~/.config/ || { echo "Error: no se pudieron copiar los archivos de configuración de nwg-look."; exit 1; } 2>&1 | tee -a "$LOG"
    ...
    cp -r ~/hypramigo/configs/Thunar ~/.config/ || { echo "Error: no se pudieron copiar los archivos de configuración de Thunar."; exit 1; } 2>&1 | tee -a "$LOG"
    cp -r ~/hypramigo/configs/wlogout ~/.config/ || { echo "Error: no se pudieron copiar los archivos de configuración de wlogout."; exit 1; } 2>&1 | tee -a "$LOG"
    cp -r ~/hypramigo/configs/xfce4 ~/.config/ || { echo "Error: no se pudieron copiar los archivos de configuración de xfce4."; exit 1; } 2>&1 | tee -a "$LOG"
    chmod +x "$HOME/.config/hypr/waybar/scripts/mediaplayer.py" "$HOME/.config/hypr/waybar/scripts/wlrecord.sh"
    chmod +x ~/.config/hypr/waybar/scripts/playerctl/playerctl.sh
...
ln -sf ~/.config/hypr/waybar/style/style-dark.css ~/.config/hypr/waybar/style.css
ln -sf ~/.config/hypr/wofi/style/style-dark.css ~/.config/hypr/wofi/style.css
...
read -n1 -rep "$OPC ¿Le gustaría reiniciar el sistema ahora? (s,n)" Rboot
