...
if pacman -Qs waybar > /dev/null; then
    read -n1 -rep "$NOTA - Waybar detectada. ¿Le gustaría desinstalar waybar e instalar waybar-hyprland-git? (s,n)" wb_git
    if [[ $wb_git =~ ^[Ss]$ ]]; then
      sudo pacman -R --noconfirm waybar 2>> "$LOG" > /dev/null || true
    fi
    printf "$NOTA - Instalando waybar-hyprland-git...\n"
    for wayb in sddm-git; do
...
read -n1 -rep "$OPC - ¿Le gustaría instalar lo scomponentes necesarios para hyprland? (s,n) " INST
if [[ $INST =~ ^[Ss]$ ]]; then
    #Paqueteria
...
read -n1 -rep "$OPC - ¿Le gustaría copiar archivos de configuración? (y,n) " CFG
if [[ $CFG == "Y" || $CFG == "y" ]]; then
    echo -e "$NOTA - Copiando archivos de configuración..."
    for DIR in hypr kitty swaylock wlogout btop cava swappy Thunar xfce4 
    do 
        DIRPATH=~/.config/$DIR
        if [ -d "$DIRPATH" ]; then 
...
read -n1 -rep "$OPC - Para portátiles ASUS ROG: ¿Le gustaría instalar el soporte de software Asus ROG? (y,n) " ROG
if [[ $ROG == "Y" || $ROG == "y" ]]; then
